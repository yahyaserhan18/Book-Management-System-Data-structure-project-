...
public:
	...
	/////////
	// للوصول إلى العقدة
	typename clsDblLinkedList<T>::Node* getNodeByIndex(int index) {
		return _QueueList.getNodeByIndex(index);
	}

	// لحذف عقدة عبر إندكس
	bool deleteNodeByIndex(int index) {
		typename clsDblLinkedList<T>::Node* node = _QueueList.getNodeByIndex(index);
		return _QueueList.deleteNode(node);
	}

	// ✅ جديد: حذف عقدة مباشرة
	bool deleteNode(typename clsDblLinkedList<T>::Node* node) {
		return _QueueList.deleteNode(node);
	}
	bool isEmpty() {
